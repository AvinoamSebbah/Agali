...
class Engine(ScraperStatus, ABC):
    ...
    def apply_limit(
        ...
        Logger.info(f"[APPLY_LIMIT] Starting with {len(intreable)} files to filter")
    ...
    def _save_and_extract(self, file_link, file_save_path):
        ...
        if StopController.should_stop():
            Logger.info(f"Stop requested - skipping download of {file_link}")
            return (file_link, file_save_path, False, False, "Stopped by user", False)
