    ...
    if max_threads:
        ...
        with concurrent.futures.ThreadPoolExecutor(
            ...
            for future in concurrent.futures.as_completed(futures):
                # Check if stop requested between each task
                if StopController.should_stop():
                    Logger.info("Stop requested - cancelling remaining downloads")
        ...
        for arg in iterable:
            # Check if stop requested between each task
            if StopController.should_stop():
                Logger.info("Stop requested - stopping sequential execution")
                break
            results.append(function_to_execute(arg))
        return results
